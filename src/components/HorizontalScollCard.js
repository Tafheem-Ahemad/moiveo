...
function HorizontalScollCard({data = [], heading, trending,media_type}) {
	const contaierRef = useRef()

    const handleNext = ()=>{
        contaierRef.current.scrollLeft += 250
    }
  ...
  return (
		<div className='container mx-auto px-3 my-10'>
          ...
          <div className=' relative'>
				...
				{/* card Components */}
                <div  ref={contaierRef} className='grid grid-cols-[repeat(auto-fit,230px)] grid-flow-col gap-5 overflow-hidden overflow-x-scroll relative z-10 scroll-smooth transition-all scrolbar-none'>
                    {
                        data.map((data,index)=>{
                        return (
